/* Equatorial Division Plane — analysis model.
 * A single-plane fork of the Division Plane projects: the one plane per zygote is the
 * EQUATORIAL plane — through the cell centre of mass, perpendicular to the animal–vegetal
 * (polar-body) axis. It splits each gene's segment-1 transcripts into an animal (polar-body)
 * half and a vegetal half; the 2-D views are meridional (equatorial plane vertical at x=0).
 * All statistics are pre-computed (build_equatorial.py); the UI only reads them.
 */
(() => {
  ...
  const BESTKEY_LABEL = { pVol: "equatorial", pCnt: "equatorial", diffVol: "equatorial", diffCnt: "equatorial" };
  ...
  (async function init() {
    try {
      ...
      // single equatorial plane (selector hidden)
      planeSelect.innerHTML = `<option value="0">Equatorial plane</option>`;
      state.planeIdx = 0;
  ...
  async function selectEmbryo(id) {
    ...
    try {
      const scene = await V.loadGz(`data/equatorial/${id}.json.gz`);
  ...
  function render() {
    ...
    if (planeShow.checked)
      traces.push(planeQuad(k, PLANE_C, 0.28, `Equatorial plane`, 41000));
    // cell centre of mass — the equatorial plane passes through it
  ...
  function renderChart() {
    ...
    chartSub.textContent = `· ${g} · equatorial plane`;
  ...
  function ensureAgg() {
    ...
    if (!state._aggCircP) state._aggCircP = V.loadGz("data/equatorial_cross_circ.json.gz")
      .then((a) => { state.aggCirc = a; }).catch(() => {});
    // (no sperm-concordance grid here: the sperm defines the plane, so it has no side)
    state._aggP = V.loadGz("data/equatorial_cross.json.gz").then((agg) => {
      state.agg = agg;
      return agg;
    });
    return state._aggP;
  ...
  function renderCurrentCrossSection() {
    ...
    xsCaption.textContent = `· ${s.id} · ${g} · equatorial plane`;
    plotInto(xsOutlines, traces, {
      dragmode: "pan", margin: { l: 42, r: 10, t: 8, b: 38 }, autosize: true,
      xaxis: { title: { text: "Distance from equatorial plane (µm) · + = animal (PB) side", font: { size: 10 } }, range: [-lim, lim],
        scaleanchor: "y", scaleratio: 1, gridcolor: "#eef1f5", zeroline: false, tickfont: { size: 9 } },
      yaxis: { title: { text: "Equatorial chord (µm)", font: { size: 10 } }, range: [-lim, lim],
  ...
  // This gene's own split at the equatorial plane. `gb[gene]` holds each gene's best plane,
  ...
  function renderGammaMu() {
    ...
    if (xsGmNote) xsGmNote.innerHTML = m.isNull && !m.hasGp
      ...
      : showingNull
        ? `<b>Negative control</b> — no anchor gene: every zygote keeps its equatorial plane but is given a ` +
        ...
        : `Each zygote is split at its <b>sperm</b> plane; the half with the higher ` +
  ...
  function wireControls() {
    ...
    [axisShow, planeShow, allShow, spermShow].forEach((c) => c.addEventListener("change", () => render()));
