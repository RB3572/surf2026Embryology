/* Extended Pseudotime Analysis — one pseudotime axis across THREE stages:
 *   zygote (ordered by the pronuclei-distance clock) → early-2-cell → late-2-cell.
 * Built on viewer-core.js (VCore). Almost identical to the Pronuclei project, but the x-axis is
 * an "extended pseudotime": each stage is a block (zygote 0–1, early-2C 1–2, late-2C 2–3) and
 * embryos are ordered WITHIN each block — zygotes by their pronuclei distance (the real geometric
 * clock), and 2-cell embryos (which have no pronuclei) by total transcript count as a maturation
 * proxy. Same dynamics as Pronuclei: per-gene / total / gene-set scatters vs pseudotime, a
 * user-selectable regression model, full stats, tabs + resize + hi-res download, and a right-drawer
 * ranking of genes by their correlation with pseudotime. Precompute in build_extpt.py. */
(() => {
  const $ = (s) => document.querySelector(s);
  const V = window.VCore;
  const LINE_C = "#111827", CUR_C = "#0891b2", DOT_C = "#f59e0b";
  const MIN_ZYG = 5;                    // min embryos containing a gene to rank/correlate it
  // stages along the pseudotime axis
  const STAGE_IDX = { zygote: 0, e2c: 1, l2c: 2 };
  const STAGE_NAME = { zygote: "Zygote", e2c: "Early 2-cell", l2c: "Late 2-cell" };
  const STAGE_COLOR = { zygote: "#0891b2", e2c: "#7c3aed", l2c: "#f97316" };
  const STAGE_BAND = { zygote: "rgba(8,145,178,0.06)", e2c: "rgba(124,58,237,0.06)", l2c: "rgba(249,115,22,0.07)" };
  ...
  (async function init() {
    try {
      ...
      const c = { zygote: 0, e2c: 0, l2c: 0 }; state.points.forEach((p) => (c[p.stage]++));
      countEl.textContent = `${m.embryos.length} embryos · ${c.zygote} zygote → ${c.e2c} early-2C → ${c.l2c} late-2C`;
      computeGeneCorr();
      populateGenes();
  ...
  function computePseudotime() {
    ...
    for (const st in groups) {
      ...
      const n = arr.length;
      arr.forEach((p, i) => {
        p.pt = STAGE_IDX[st] + (n <= 1 ? 0.5 : 0.06 + 0.88 * (i / (n - 1)));
  ...
  function geneSeries(g) {
    const xs = [], ys = [], ids = [], labels = [];   // xs = gene count, ys = extended pseudotime
    for (const e of state.genesAgg.embryos) {
      const c = e.genes[g]; if (c == null) continue;
      xs.push(c); ys.push(e.pt); ids.push(e.id); labels.push((state.byId[e.id] || {}).label || e.id);
    }
  ...
  async function selectEmbryo(id) {
    ...
    try {
      // zygote scenes come from the pronuclei project (pronuclei + min-line); 2-cell from extpt
  ...
  function render() {
    ...
    const isZyg = Array.isArray(s.pron_labels) && s.pron_labels.length === 2;   // zygote scene has pronuclei
    const [la, lb] = isZyg ? s.pron_labels : [null, null];
    const pcolor = isZyg ? { [la]: "#2563eb", [lb]: "#dc2626" } : {};
    const traces = [];
    for (const lbl of s.mask_labels) {
      const pron = isZyg && (lbl === la || lbl === lb);
      const t = segMesh(s, lbl, pron ? pcolor[lbl] : "#9aa3b2", pron ? 0.5 : 0.1,
  ...
  function renderReadout(meta) {
    const s = state.scene;
    const isZyg = Array.isArray(s.pron_labels) && s.pron_labels.length === 2;
    ...
    pnReadout.innerHTML =
      ...
      (isZyg ? `<div class="pn-resid">pronuclei = segments <b>${s.pron_labels[0]}</b> &amp; <b>${s.pron_labels[1]}</b></div>` : "");
  }
  ...
  function renderSetChips() {
    ...
    setChipsEl.innerHTML = state.geneSet.map((g) => {
      ...
      return `<span class="pn-set-chip${ok ? "" : " absent"}" title="${ok ? g : g + " — not in any embryo's panel"}">${g}` +
  ...
  // x = extended pseudotime (0–3), y = count. Points coloured + legended by stage; three faint
  // stage bands + stage tick labels sit under the pseudotime axis. flip transposes the view.
  function scatter(div, xs, ys, ids, labels, model, xTitle, yTitle, yUnit, curId, xUnit, yFmt) {
    xUnit = xUnit == null ? "" : xUnit;
    yFmt = yFmt || ",";
    const flip = state.flip;
    const xmin = Math.min(...xs), xmax = Math.max(...xs);
    const cxD = [], cyP = [], N = 120;             // smooth model curve, sampled over pseudotime
    ...
    const pts = xs.map((x, i) => ({ h: H(x, ys[i]), v: Vv(x, ys[i]), lab: labels[i], id: ids[i], stage: (state.byId[ids[i]] || {}).stage }));
    const cur = pts.find((o) => o.id === curId);
    const chx = cxD.map((d, i) => H(d, cyP[i])), chy = cxD.map((d, i) => Vv(d, cyP[i]));
    const hUnit = flip ? yUnit : xUnit, vUnit = flip ? xUnit : yUnit;
    const hFmt = flip ? yFmt : ".2f", vFmt = flip ? ".2f" : yFmt;
    ...
    const traces = [];
    ["zygote", "e2c", "l2c"].forEach((st) => {      // one marker trace per stage (colour + legend)
      const g = pts.filter((o) => o.stage === st && o.id !== curId); if (!g.length) return;
      traces.push({ type: "scatter", mode: "markers", name: STAGE_NAME[st], x: g.map((o) => o.h), y: g.map((o) => o.v),
        marker: { size: 7, color: STAGE_COLOR[st], opacity: 0.66, line: { width: 0 } },
        text: g.map((o) => o.lab), hovertemplate: hover });
    });
    traces.push({ type: "scatter", mode: "lines", name: model.label, x: chx, y: chy,
      line: { color: "#111827", width: 2.4, shape: "spline" }, hoverinfo: "skip" });
    if (cur) traces.push({ type: "scatter", mode: "markers", name: cur.lab, x: [cur.h], y: [cur.v], showlegend: false,
      marker: { size: 15, color: STAGE_COLOR[cur.stage] || CUR_C, line: { width: 2.5, color: "#fff" } },
      text: [cur.lab], hovertemplate: hover });
    const ptOnX = !flip;
    const shapes = ["zygote", "e2c", "l2c"].map((st) => { const a = STAGE_IDX[st];
      return ptOnX
        ? { type: "rect", xref: "x", yref: "paper", x0: a, x1: a + 1, y0: 0, y1: 1, fillcolor: STAGE_BAND[st], line: { width: 0 }, layer: "below" }
        : { type: "rect", xref: "paper", yref: "y", y0: a, y1: a + 1, x0: 0, x1: 1, fillcolor: STAGE_BAND[st], line: { width: 0 }, layer: "below" }; });
    const ptAxis = { tickmode: "array", tickvals: [0.5, 1.5, 2.5], ticktext: ["Zygote", "Early 2-cell", "Late 2-cell"], range: [-0.03, 3.03] };
    const xaxis = { title: { text: hTitle, font: { size: 11 } }, tickfont: { size: 10 }, gridcolor: "#eef1f5", zeroline: false };
    const yaxis = { title: { text: vTitle, font: { size: 10 } }, tickfont: { size: 9 }, gridcolor: "#eef1f5" };
    if (ptOnX) { Object.assign(xaxis, ptAxis); yaxis.rangemode = "tozero"; }
    else { Object.assign(yaxis, ptAxis); xaxis.rangemode = "tozero"; }
    plotInto(div, traces, {
      margin: { l: 64, r: 12, t: 6, b: 42 }, autosize: true, shapes, xaxis, yaxis,
  ...
  function renderScatter() {
    ...
    const pts = state.points, X = pts.map((p) => p.pt);
  ...
  function renderGeneScatter() {
    ...
    const g = gene();
    const xs = [], ys = [], ids = [], labels = [];   // this gene's count vs extended pseudotime
    for (const e of state.genesAgg.embryos) {
      if (!(g in e.genes) || e.pt == null) continue;
      xs.push(e.pt); ys.push(normVal(geneCountIn(e.id, g), e.id));
      ids.push(e.id); labels.push((state.byId[e.id] || {}).label || e.id);
    }
    if (xs.length < 2) {
      Plotly.purge(geneScatter); geneScatter.classList.remove("js-plotly-plot");
      geneScatter.innerHTML = `<div class="pn-empty">Only ${xs.length} embryo${xs.length === 1 ? "" : "s"} contain${xs.length === 1 ? "s" : ""} <b>${g}</b> — too few to fit.</div>`;
  ...
  // "require all" → only embryos that contain every set gene; otherwise sum whichever are present.
  function setSeries() {
    const set = state.geneSet, out = { xs: [], ys: [], ids: [], labels: [] };
    if (!set.length) return out;
    for (const p of state.points) {
      const e = state.gaById[p.id]; if (!e || e.pt == null) continue;
      ...
      out.xs.push(p.pt); out.ys.push(normVal(sum, p.id)); out.ids.push(p.id); out.labels.push(p.label);
    }
    return out;
  }
  function renderSetScatter() {
    ...
    if (nSet === 0) {
      ...
      setScatter.innerHTML = `<div class="pn-empty">Add genes to the set (or pick a preset) to plot the summed count vs extended pseudotime.</div>`;
    ...
    if (s.xs.length < 2) {
      ...
      const why = state.setRequireAll
        ? "no embryo contains all of them — panels are disjoint, so turn off “require all genes”"
        : "no embryo contains any of them";
      setScatter.innerHTML = `<div class="pn-empty">Only ${s.xs.length} embryo${s.xs.length === 1 ? "" : "s"} plottable — ${why}.</div>`;
  ...
  function rankRows(rows) {
    ...
    return html || `<div class="pn-empty">No genes in ≥ ${MIN_ZYG} embryos.</div>`;
  }
