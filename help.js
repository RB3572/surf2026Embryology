...
(() => {
  ...
  const HELP = {
    ...
    "pseudosperm": { eyebrow: "Pseudosperm plane · the inverse question", title: "Where would the sperm have to be?",
      body: [
        "The Sperm-Defined Division Plane project cuts a zygote with the one plane through <b>the sperm, the polar-body COM and the cell COM</b>. Because that plane contains the polar axis, it is a member of exactly the family the Division Plane Sweep rotates through — the sperm's only job is to pick the <b>angle</b>. 30 of our zygotes have a labelled sperm and therefore an angle. 20 do not.",
        "<b>This page runs it backwards.</b> The right drawer ranks genes by how sharply they split across the <i>real</i> sperm plane, pooled over every sperm-carrying zygote that measures them. Tick some, and for a zygote with no sperm the page sweeps all 180 angles and keeps the one that best reproduces that split. That is the <b>pseudosperm plane</b>, and the in-plane direction perpendicular to the polar axis is where a sperm would have had to be.",
        "<b>⚠️ A pseudosperm plane is fitted, not observed.</b> It is evidence about the geometry of the genes you selected, and nothing more. It is not a located sperm, and it must never be quoted as one.",
        "<b>The Validation tab is the point.</b> The same fit is run on the zygotes that DO have a sperm, with the sperm ignored, and the answer compared with the plane the sperm actually defines. Two planes drawn at random differ by 45° on average, so <b>45° is the null, not 0°</b>. If the fit does not beat 45°, the idea does not work for those genes — and the panel will say so.",
        "<b>Nothing from a pronucleus or the polar body enters any number here.</b> Only segment 1 — the cytoplasm, with the pronuclei carved out of it — is ever counted, and the per-side volume that turns a count into a concentration is that same segment's volume on that side. The exclusion is structural, not a filter that could be forgotten.",
        "<b>The ranking.</b> Per zygote, a two-sided <b>exact binomial</b> test of the side-A count against that cell's own volume split volA/(volA+volB) — so a gene is called asymmetric only if it beats what the geometry alone would give. Across zygotes, <b>Fisher</b> combines them ('is it asymmetric?'), while the <b>Stouffer Z</b> asks the far stronger question 'is it asymmetric the <i>same way</i> every time?'. A gene with a huge Fisher score and a Z near zero is asymmetric in every embryo in a different direction, which is a different — and much weaker — claim.",
        "<b>Read the sweep curve, not just the peak.</b> A broad flat curve means the angle is barely determined and the single number at the top is close to meaningless. The curve is on the page so that cannot be hidden.",
        "<b>One approximation, and where it bites.</b> The original voxel masks are not in this repo, so the cytoplasm volume either side of an <i>arbitrary</i> plane is recomputed by clipping the segment-1 mesh, corrected onto the 18 angles where the exact voxel volumes are known. Held out against the 30 real sperm planes it is right to about 0.3% of the volume split. The sperm planes themselves use the exact volumes. Switching the measure to <b>count fraction</b> removes volume from the calculation entirely.",
        "<b>Who is missing.</b> 50 zygotes have a detectable polar body and therefore a polar axis. Three sperm-carrying zygotes do not, so no plane in this family exists for them at all and they are absent rather than silently defaulted.",
      ] },
