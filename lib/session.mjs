...
/** Is this user an admin? Explicitly listed, or a lab admin/PI in Lab Logger. */
export function isAdminUser(userId, labRole) {
  const ids = (process.env.ADMIN_USER_IDS || "")
    .split(",").map((s) => s.trim()).filter(Boolean);
  return ids.includes(userId) || labRole === "admin" || labRole === "pi";
}
