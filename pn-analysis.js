/* pn-analysis.js — shared transcript-vs-clock analysis core (window.PNA).
 *
 * The DOM-independent statistics + regression models used by the "transcripts vs a
 * pronuclear clock" viewers. Extracted verbatim from the (proven) pronuclei.js code so a
 * second page can render the SAME 10-model menu, R²/Pearson/permutation statistics and
 * fit curves against any x-axis (here: pseudotime τ) without duplicating the maths.
 *
 * The only generalisation vs the original is statsHtml(xs, ys, fit, xName): the x-variable
 * name in the p-value tooltip is a parameter instead of the hard-coded "pronuclei distance".
 */
window.PNA = (function () {
  "use strict";
  ...
  // full stat line shared by every scatter: model · equation · R² · Pearson r · p (+ null cross-check in tooltip)
  function statsHtml(xs, ys, fit, xName) {
    const x = xName || "x";
    ...
    const tip = `p tests H0: ${x} & count are uncorrelated (two-sided). ` +
  ...
  // ---------- regression models: y (count / concentration) vs x (a clock) ----------
  ...
  function solve3(A, d) {
    ...
    for (let c = 0; c < 3; c++) { let p = c;
      ...
      for (let r = 0; r < 3; r++) { if (r === c) continue; const fct = A[r][c] / A[c][c]; for (let k = c; k < 4; k++) A[r][k] -= fct * A[c][k]; } }
  ...
  function irls(xs, ys, fam, init, iter = 60) {
    let [b0, b1] = init;
  ...
  const MODELS = {
    ...
    logistic: { label: "Logistic (sigmoid)", scale: "logit", bio: "a saturating switch — the shape of zygotic genome activation (transcription turns on, then plateaus).",
      fit(xs, ys) { const L = 1.02 * Math.max(...ys); const px = [], py = []; for (let i = 0; i < xs.length; i++) { const q = clmp(ys[i] / L, 0.001, 0.999); px.push(xs[i]); py.push(Math.log(q / (1 - q))); }
        const [a, b] = wls(px, py);
        return { predict: (x) => L / (1 + Math.exp(-clmp(a + b * x, -30, 30))), r2: r2lin(px, py), params: `L=${sci(L)}, k=${b.toFixed(3)}, x₀=${b ? (-a / b).toFixed(3) : "–"}` }; } },
  ...
  return { MODELS, SCALE_LABEL, fitModel, statsHtml, linreg, pearsonR, pearsonP, permP, fmtP, pStars, spearman, sci };
})();
