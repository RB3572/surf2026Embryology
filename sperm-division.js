...
(() => {
  ...
  const XY = 0.15;
  const AXIS_C = "#111827", PLANE_C = "#0d9488";
  const BLUE = "#2166ac", RED = "#b2182b", GREEN = "#16a34a";
  const SPERM_C = V.SPERM_COLOR, COM_C = "#6d28d9";   // sperm marker + cell-COM / construction lines
  ...
  const BESTKEY_LABEL = { pVol: "sperm", pCnt: "sperm", diffVol: "sperm", diffCnt: "sperm" };
  ...
  function render() {
    ...
    if (planeShow.checked)
      traces.push(planeQuad(k, PLANE_C, 0.28, `Sperm plane`, 41000));
    // sperm + how the plane is constructed: cell COM → sperm and cell COM → polar-body COM
    if (spermShow.checked && A.sperm_plot) {
      const c = A.com_plot, spp = A.sperm_plot, pbp = A.pb_plot;
      traces.push({ type: "scatter3d", mode: "lines", name: "cell COM → sperm",
        x: [c[0], spp[0]], y: [c[1], spp[1]], z: [c[2], spp[2]],
        line: { color: SPERM_C, width: 5 }, hovertemplate: "cell COM → sperm<extra></extra>", legendrank: 41500 });
      if (pbp) traces.push({ type: "scatter3d", mode: "lines", name: "cell COM → polar body",
        x: [c[0], pbp[0]], y: [c[1], pbp[1]], z: [c[2], pbp[2]],
        line: { color: COM_C, width: 5 }, hovertemplate: "cell COM → polar body<extra></extra>", legendrank: 41501 });
      traces.push({ type: "scatter3d", mode: "markers", name: "sperm",
        x: [spp[0]], y: [spp[1]], z: [spp[2]],
        marker: { size: 7, color: SPERM_C, symbol: "diamond", line: { width: 1, color: "#fff" } },
        hovertemplate: "sperm<extra></extra>", legendrank: 41502 });
      traces.push({ type: "scatter3d", mode: "markers", name: "cell COM",
        x: [c[0]], y: [c[1]], z: [c[2]],
        marker: { size: 6, color: COM_C, symbol: "circle", line: { width: 1, color: "#fff" } },
        hovertemplate: "cell centre of mass (segment 1)<extra></extra>", legendrank: 41503 });
  ...
  function ensureAgg() {
    ...
    if (!state._aggCircP) state._aggCircP = V.loadGz("data/sperm_division_cross_circ.json.gz")
      .then((a) => { state.aggCirc = a; }).catch(() => {});
    // (no sperm-concordance grid here: the sperm defines the plane, so it has no side)
    state._aggP = V.loadGz("data/sperm_division_cross.json.gz").then((agg) => {
      state.agg = agg;
      return agg;
    });
    return state._aggP;
  ...
  function renderCurrentCrossSection() {
    ...
    xsCaption.textContent = `· ${s.id} · ${g} · sperm plane`;
  ...
  function renderGammaMu() {
    ...
    if (xsGmNote) xsGmNote.innerHTML = m.isNull && !m.hasGp
      ...
      : showingNull
        ? `<b>Negative control</b> — no anchor gene: every zygote keeps its sperm plane but is given a ` +
